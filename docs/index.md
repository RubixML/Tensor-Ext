...
Tensor is a PHP extension for scientific computing. It provides an object-oriented API consisting of a `Tensor` interface (composed of several sub-interfaces) implemented by three concrete classes, plus decomposition and reduction result objects.
...
| [TensorBuffer](TensorBuffer.md) | A decorator that wraps the kernel `Buffer` class and adds sorting and slicing. |
...
- **Native math.** Operations route into native C (OpenBLAS/LAPACKE) for performance.
