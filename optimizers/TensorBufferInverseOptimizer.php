...
class TensorBufferInverseOptimizer extends OptimizerAbstract
{
    /**
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (count($expression['parameters']) !== 2) {
            throw new CompilerException(
                'Buffer Inverse accepts exactly 2 arguments, ' . count($expression['parameters']) . ' given.',
                $expression
            );
        }
        ...
        $context->codePrinter->output(
            "tensor_buffer_inverse($symbol, {$resolvedParams[0]}, {$resolvedParams[1]});"
        );
