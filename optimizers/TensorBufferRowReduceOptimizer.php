...
class TensorBufferRowReduceOptimizer extends OptimizerAbstract
{
    /**
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (count($expression['parameters']) !== 3) {
            throw new CompilerException(
                'Buffer row reduce accepts exactly 3 arguments, ' . count($expression['parameters']) . ' given.',
                $expression
            );
        }
        ...
        $context->codePrinter->output(
            "tensor_buffer_row_reduce($symbol, {$resolvedParams[0]}, {$resolvedParams[1]}, {$resolvedParams[2]});"
        );
