...
class TensorAbsOptimizer extends OptimizerAbstract
{
    /**
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (count($expression['parameters']) !== 1) {
            throw new CompilerException(
                'Tensor abs accepts exactly 1 argument, ' . count($expression['parameters']) . 'given.',
                $expression
            );
        }
        ...
        $context->codePrinter->output(
            "tensor_abs($symbol, {$resolvedParams[0]});"
        );
