...
class TensorLessEqualColOptimizer extends OptimizerAbstract
{
    /**
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (count($expression['parameters']) !== 3) {
            throw new CompilerException(
                'Tensor less equal col accepts exactly three arguments, ' . count($expression['parameters']) . 'given.',
                $expression
            );
        }
        ...
        $context->codePrinter->output(
            "tensor_less_equal_col($symbol, {$resolvedParams[0]}, {$resolvedParams[1]}, {$resolvedParams[2]});"
        );
