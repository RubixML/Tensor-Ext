...
class TensorBufferBinaryScalarOptimizer extends OptimizerAbstract
{
    /**
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (count($expression['parameters']) !== 3) {
            throw new CompilerException(
                'Buffer binary scalar accepts exactly 3 arguments, ' . count($expression['parameters']) . ' given.',
                $expression
            );
        }
        ...
        $context->codePrinter->output(
            "tensor_buffer_binary_scalar($symbol, {$resolvedParams[0]}, {$resolvedParams[1]}, {$resolvedParams[2]});"
        );
