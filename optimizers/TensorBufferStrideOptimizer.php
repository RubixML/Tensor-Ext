...
class TensorBufferStrideOptimizer extends OptimizerAbstract
{
    /**
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (count($expression['parameters']) !== 4) {
            throw new CompilerException(
                'Buffer stride accepts exactly 4 arguments, ' . count($expression['parameters']) . ' given.',
                $expression
            );
        }
        ...
        $context->codePrinter->output(
            "tensor_buffer_stride($symbol, {$resolvedParams[0]}, {$resolvedParams[1]}, {$resolvedParams[2]}, {$resolvedParams[3]});"
        );
