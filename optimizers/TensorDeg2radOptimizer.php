...
class TensorDeg2radOptimizer extends OptimizerAbstract
{
    /**
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (count($expression['parameters']) !== 1) {
            throw new CompilerException(
                'Tensor deg2rad accepts exactly 1 argument, ' . count($expression['parameters']) . 'given.',
                $expression
            );
        }
        ...
        $context->codePrinter->output(
            "tensor_deg2rad($symbol, {$resolvedParams[0]});"
        );
