...
class TensorBufferArgmaxOptimizer extends OptimizerAbstract
{
    /**
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (count($expression['parameters']) !== 1) {
            throw new CompilerException(
                'Tensor buffer argmax accepts exactly one argument, ' . count($expression['parameters']) . 'given.',
                $expression
            );
        }
        ...
        $context->codePrinter->output(
            "tensor_buffer_argmax($symbol, {$resolvedParams[0]});"
        );
