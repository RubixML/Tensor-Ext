...
class TensorModRowOptimizer extends OptimizerAbstract
{
    /**
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (count($expression['parameters']) !== 3) {
            throw new CompilerException(
                'Tensor mod row accepts exactly three arguments, ' . count($expression['parameters']) . 'given.',
                $expression
            );
        }
        ...
        $context->codePrinter->output(
            "tensor_mod_row($symbol, {$resolvedParams[0]}, {$resolvedParams[1]}, {$resolvedParams[2]});"
        );
