...
class TensorAsinOptimizer extends OptimizerAbstract
{
    /**
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (count($expression['parameters']) !== 1) {
            throw new CompilerException(
                'Tensor asin accepts exactly 1 argument, ' . count($expression['parameters']) . 'given.',
                $expression
            );
        }
        ...
        $context->codePrinter->output(
            "tensor_asin($symbol, {$resolvedParams[0]});"
        );
