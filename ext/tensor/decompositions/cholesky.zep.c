...
PHP_METHOD(Tensor_Decompositions_Cholesky, decompose)
{
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 18, &_5);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Tensor_Decompositions_Cholesky, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 7, l);
}
...
PHP_METHOD(Tensor_Decompositions_Cholesky, lT)
{
	...
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 7, PH_NOISY_CC | PH_READONLY);
