...
PHP_METHOD(Tensor_TensorBuffer, fromBuffers)
{
	...
	zval *buffers_param = NULL, _2, _3, _4, _0$$3, _1$$4;
	zval buffers;
	...
	if (UNEXPECTED(rows < 1)) {
		...
		array_init(&_0$$3);
		tensor_buffer_from_array(return_value, &_0$$3);
		RETURN_MM();
	}
	if (UNEXPECTED(rows == 1)) {
	...
	ZVAL_LONG(&_3, 1);
	ZEPHIR_CALL_FUNCTION(&_4, "array_slice", NULL, 30, &buffers, &_3);
...
PHP_METHOD(Tensor_TensorBuffer, slice)
{
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 31, &b);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Tensor_TensorBuffer, split)
{
	...
	if (Z_TYPE_P(_6) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(_6), _8)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, &_9$$4, "__construct", &_10, 31, &buffer);
			zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(NULL, &_13$$5, "__construct", &_10, 31, &buffer);
				zephir_check_call_status();
